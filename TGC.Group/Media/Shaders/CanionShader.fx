...
VS_OUTPUT_DIFFUSE_MAP vs_DiffuseMap(VS_INPUT_DIFFUSE_MAP input)
{
    ...
    output.WorldCoord = mul(input.Position, matWorld); // o skinPosition.xyz
...
float4 ps_DiffuseMap(PS_DIFFUSE_MAP input) : COLOR0
{
    ...
    //if (input.WorldCoord.y > 150 * sin(time))
     //   discard;

    //if (input.WorldCoord.y % 2 < 1)
      //  discard;
    //else
